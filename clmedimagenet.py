...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.class_head = nn.Sequential(
            OrderedDict(
                [
                    ("fc1", nn.Linear(in_channels+6, 512)),
                    ("relu1", get_act_layer(name=act)),
                    ("fc2", nn.Linear(512, 256)),
                    ("relu2", get_act_layer(name=act)),
                    ("out", nn.Linear(256 ,out_channels)),
                ]
            )
        )
    ...
    def forward(self, x: torch.Tensor,linchuang) -> torch.Tensor:
        x = self.features(x)
        x = self.class_layers(x)
        x=torch.cat((x,linchuang),dim=1)
        x = self.class_head(x)
        return x
...
class Clmedimagenet121(DenseNet):
