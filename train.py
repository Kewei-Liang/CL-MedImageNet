...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in tqdm(train_loader):
            step += 1
            inputs, labels,linchuang ,filename= batch_data[0].to(device), batch_data[1].to(device), batch_data[2].to(device),batch_data[3]
            optimizer.zero_grad()
            outputs = model(inputs,linchuang)
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            logging.info(f"Current epoch: {epoch+1} current confusion : {C2} ")
        
            if metric > best_metric:
                best_metric = metric
                best_metric_epoch = epoch + 1
                torch.save(model.state_dict(), r"D:\best_metric_model_classification3d_array.pth")
                logging.info("saved new best metric model")
            logging.info(f"Current epoch: {epoch+1} current AUC : {metric:.4f} ")
            logging.info(f"Best AUC: {best_metric:.4f} at epoch {best_metric_epoch}")
