...
class ImageDataset(Dataset, Randomizable):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        import json
        filename=r"D:\linchuang.json"
        with open(filename, 'r') as f:
            # 使用json.load()方法从文件中读取数据
            data = json.load(f)
    ...
    def __getitem__(self, index: int):
        ...
        directory, filename = os.path.split(self.image_files[index])
        filename=filename.replace(".nii.gz","")
        linchuang=self.linchuang[filename]
        x_linchuang = torch.empty(6)
        lc_list=linchuang.split("_")
        x_linchuang[0]=float(lc_list[0])
        x_linchuang[1]=float(lc_list[1])
        x_linchuang[2]=float(lc_list[3])
        x_linchuang[3]=float(lc_list[4])
        x_linchuang[4]=float(lc_list[5])
        x_linchuang[5]=float(lc_list[6])
        if self.image_only:
        ...
        if len(data) == 1:
            return data[0]
        data.append(self.image_files[index])
